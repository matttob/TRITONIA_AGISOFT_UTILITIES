...
for chunk in Metashape.app.document.chunks:
    ...
    # intitialise Projection Accuracy filter
    f.init(chunk, Metashape.PointCloud.Filter.ProjectionAccuracy)
    # create numpy array of values of Projection Accuracy using the min and max value provided by the filter
    projacc_iter = np.arange(f.min_value, f.max_value,((f.max_value-f.min_value)/number_of_err_steps))
    
    #initialise list for storing values of the percentage of points selected for each value of Projection Accuracy
    percent_selecteds =[]
    

    # loop through all values of Projection Accuracy and store the number of points selected in each case
    for i in projacc_iter:
        # select points 
        f.selectPoints(i)
    ...
    # apply same selection criteria to projacc_iter array to ensure correct indexing
    projacc_iter_less_than_half = projacc_iter[percent_selecteds<target_percentage]
    ...
    # remove points using the projacc_iter value with the index defined above
    f.removePoints(projacc_iter_less_than_half[rep_cunc_half_points_selected_ind])
